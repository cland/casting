...
<div class="fieldcontain ${hasErrors(bean: portfolioInstance, field: 'comments', 'error')} ">
	...
	<g:textArea name="comments" value="${portfolioInstance?.comments}" rows="3" cols="40"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: portfolioInstance, field: 'profiles', 'error')} ">
	...
	<g:select name="profiles" from="${portfolioInstance?.production?.profiles?.sort{it.name}}" multiple="multiple" optionKey="id" size="5" value="${portfolioInstance?.profiles*.id}" optionValue="castingDetails" class="many-to-many multi-list-picker-large"/>
</div>
<br/>
